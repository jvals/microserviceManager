...
internal class DockerComposeServiceTest {
    ...
    fun `If one service in the input is a NOOP, the output should contain all services except the NOOP service`() {
        ...
        val runConfigurations = mapOf(
            ServiceName(name = "projectA") to RunConfiguration(mode = RunMode.NOOP, context = ""),
            ServiceName(name = "projectB") to RunConfiguration(mode = RunMode.RUN, context = "")
        )
    ...
    fun `Applying a connection should result in a new entry in the environment variables`() {
        ...
        val connectionConfigurations = mapOf(
            ServiceName("projectA") to ConnectionConf(
                mapOf(
                    ServiceName("projectB") to Connection(
                        internalConnections = listOf(EnvironmentVar("PROJECT_B_PORT", "8081"))
                    )
                )
            )
        )
    ...
    fun `Environment variables should be extended, not overwritten`() {
        ...
        val connectionConfigurations = mapOf(
            ServiceName("projectA") to ConnectionConf(
                mapOf(
                    ServiceName("projectB") to Connection(
                        internalConnections = listOf(EnvironmentVar("CONSTRETTO_TAGS", "projectB"))
                    ),
                    ServiceName("projectC") to Connection(
                        internalConnections = listOf(EnvironmentVar("CONSTRETTO_TAGS", "projectC"))
                    )
                )
            ),
            ServiceName("projectB") to ConnectionConf(
                mapOf(
                    ServiceName("projectA") to Connection(
                        internalConnections = listOf(EnvironmentVar("CONSTRETTO_TAGS", "projectA"))
                    ),
                    ServiceName("projectC") to Connection(
                        internalConnections = listOf(EnvironmentVar("CONSTRETTO_TAGS", "projectC"))
                    )
    ...
    @Test
    fun `Services in 'external' mode should use the external connection config`() {
        val inputServices = mapOf(
            ServiceName(name = "projectA") to Service(
                containerName = "projectA_local",
                image = Image("projectA:latest"),
        ...
        val runConfiguration = mapOf(
            ServiceName("projectA") to RunConfiguration(
                mode = RunMode.EXTERNAL,
                context = ""
            ),
            ServiceName("projectB") to RunConfiguration(
                mode = RunMode.RUN,
                context = ""
            )
        )

        val connectionConf = mapOf(
            ServiceName("projectB") to ConnectionConf(
                mapOf(
                    ServiceName("projectA") to Connection(
                        internalConnections = listOf(EnvironmentVar("PROJECT_A_HOST", "http://localhost:1234")),
                        externalConnections = listOf(EnvironmentVar("PROJECT_A_HOST", "http://host.docker.internal:1234"))
                    )
                )
            )
        )

        val expectedOutputServices = mapOf(
            ServiceName(name = "projectB") to Service(
                containerName = "projectB_local",
                image = Image("projectB:latest"),
                build = null,
                ...
                environment = mapOf(
                    "JAVA_OPTS" to "-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005",
                    "CONSTRETTO_TAGS" to "local",
                    "PROJECT_A_HOST" to "http://host.docker.internal:1234"
                ),
                memLimit = "1000M"
            )
        )
        val externalServices = DockerComposeService.getExternalServices(inputServices, RunConfigurations(runConfiguration))

        var actualOutputServices = DockerComposeService.applyRunConfiguration(inputServices, runConfiguration)
        actualOutputServices = DockerComposeService.applyConnectionConfiguration(actualOutputServices, connectionConf, externalServices)

        assertEquals(expectedOutputServices, actualOutputServices)
    }
}
